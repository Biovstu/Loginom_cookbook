...
for i,col in enumerate(ref_dif):
    if col != 'Ref_Key':
        buff_resp = requests.get(f"http://dcr-iis03/erp_data/odata/standard.odata/Catalog_Номенклатура?$select=Ref_Key,{col}", headers=dict(Accept='application/json;odata=nometadata'), auth=basic)
